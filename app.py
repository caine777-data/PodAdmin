...
from pod_api import (PodAPI, PodAPIError, EnvoiAnnule,
...
class UploadItem:
    ...
    def __init__(self, path: str):
        ...
        # été créée au nom du compte véhicule. Elle n'est plus jamais relancée
...
class App(_AppBase):
    def __init__(self):
        ...
        # Arrêt du lot de TÉLÉVERSEMENT. Distinct de `lot_interrompu` (onglet
        # Vidéos) : les deux lots peuvent tourner en même temps, et arrêter
        # l'un ne doit pas arrêter l'autre.
        self.depot_interrompu = threading.Event()
        # Vrai pendant un lot de téléversement. La boucle d'envoi parcourt
        # `self.items` par INDEX : retirer une ligne en cours de route décalait
        # la liste et faisait sauter, sans rien dire, la vidéo suivante. Pendant
        # un lot, on peut donc AJOUTER (elles partent à la suite) mais pas retirer.
        ...
        # renvoyer un fichier déjà envoyé puis retiré de la liste (doublon sur Pod).
        # Mémoire de SESSION seulement : elle disparaît à la fermeture.
        ...
        # obligatoire ci-dessous) : contrôle local immédiat — un blocage déjà
        # confirmé s'applique sans attendre le réseau — puis surveillance
        # périodique. Exécuté AVANT le contrôle de MAJ obligatoire, pour que
        # les deux mécanismes restent orthogonaux : l'un ne conditionne
        # jamais l'autre.
    ...
    def _build_tab_upload(self):
        ...
        # repos, il laisserait croire qu'il y a quelque chose à arrêter.
        # Gris et non rouge : il n'efface rien, il arrête.
        self.upload_stop_btn = ctk.CTkButton(
            launch, text="🛑  Interrompre", height=H_PRINCIPAL,
    ...
    def _maj_verrou_liste(self):
        ...
        L'ajout reste permis : un fichier ajouté pendant un lot part à la suite,
        dans le même lot."""
    ...
    def _depot_interrompre(self):
        ...
        Contrairement aux lots de l'onglet Vidéos (arrêt entre deux vidéos),
        l'arrêt porte AUSSI sur la vidéo en cours : un gros fichier peut
        prendre une demi-heure, et une attente après un 504 jusqu'à 30 min.
    ...
    def _add_paths(self, paths) -> int:
        ...
        Trois cas écartés ou signalés :
          ...
          • déjà envoyé depuis l'ouverture de l'application, puis retiré de la
            liste → confirmation : le renvoyer créerait un second exemplaire sur
            Pod. Cette mémoire est celle de la SESSION ;
    ...
    def _refresh_list(self):
        ...
        for i, it in enumerate(self.items):
            ...
            it.btn_retirer = ctk.CTkButton(
                ...
                command=lambda item=it: self._remove_item(item))
        ...
        def verifier_arret():
            if annuler and annuler():
                raise EnvoiAnnule(
                    ...
                    f"compte véhicule. Cherchez « {marqueur} » dans l'onglet Vidéos, "
                    "puis réaffectez-la (ou supprimez-la) avant de la renvoyer.",
                    a_verifier=True)
    ...
    def _deposer_par_morceaux(self, chunked, it, progress, on_retry, annuler=None):
        ...
        try:
            ...
            # retrouver la vidéo côté web (recherche « pa… » dans l'onglet Vidéos).
            self._ui(self._log,
            ...
            if not video:
                self._ui(self._log,
                         ...
                         f"après le HTTP {ce.status}. Vérifiez sur Pod (repère "
    ...
    def _replier_sur_chunked(self, it, owner_url: str, type_url: str,
                             is_draft: bool, progress, on_retry, annuler=None):
    ...
    def _do_batch_upload(self, owner_url: str, type_url: str,
        ...
        # le suit (il annonçait « 3/2 »). Les RETRAITS sont bloqués pendant le
        # lot (`depot_en_cours`) : un retrait décalait les index et faisait
        # sauter la vidéo suivante sans rien dire.
        idx = 0
    ...
    def _on_batch_done(self, ok: int, total: int, interrompu: bool = False):
        ...
        reussite = (ok == total)
        if interrompu:
            reste = sum(1 for it in self.items if not it.done and not it.a_verifier)
    ...
    #
    # Indépendant de la mise à jour obligatoire ci-dessus : ce mécanisme ne
    # dépend d'aucun numéro de version. Il est piloté depuis GitHub Actions
    # (workflow "Build installers" → champ "Blocage", PARAMÉTRÉ PAR DÉFAUT
    # SUR « ne rien changer ») et ne fait qu'écrire `etat.json` sur le dépôt
    # public — voir BLOCAGE.md. Invisible tant que rien n'est bloqué : pas de
    # bandeau, aucun message, comme pour EnqueteGen.
    ...
    def _appliquer_blocage(self, bloque: bool | None):
    ...
    def _bloquer_application(self):
        ...
        Les fenêtres secondaires déjà ouvertes (Réglages, À propos…) sont
        fermées : les laisser ouvertes par-dessus le voile permettrait de
        ...
        if self._voile_blocage is None:
            ...
            ctk.CTkLabel(centre, text="PodAdmin n'est pas disponible",
                         font=ctk.CTkFont(size=17, weight="bold"),
            ...
            # Seule issue, comme pour la MAJ obligatoire : on peut toujours
            # quitter proprement (voir la note d'incident sur `focus_force`
            # dans `_bloquer_demarrage` — même prudence ici).
    ...
    def _build_tab_help(self):
        ...
        section(
            ...
            "• Pendant un envoi, on peut AJOUTER des vidéos (elles partent à la suite, "
            "dans le même lot), mais pas en retirer : « ✕ », « Vider la liste » et "
            "« Retirer les terminées » sont grisés jusqu'à la fin.\n"
            "• Réajouter un fichier déjà envoyé depuis l'ouverture de PodAdmin demande "
            "confirmation : le renvoyer créerait un second exemplaire sur Pod.\n"
            "• 🛑 Interrompre (visible pendant l'envoi) : arrête le lot, y compris la "
            "vidéo en cours, en quelques secondes. Aucune vidéo n'est créée à moitié ; "
            "« Lancer le téléversement » reprend ensuite les vidéos restantes.\n"
            "• Une vidéo marquée « ⚠️ à vérifier » a été arrêtée pendant la finalisation "
            "serveur : elle existe peut-être déjà, au nom du compte véhicule. Elle "
            "n'est jamais renvoyée seule — le Journal indique quoi chercher dans "
            "l'onglet Vidéos. Pour la renvoyer malgré tout, retirez-la de la liste "
            "et ajoutez de nouveau le fichier.")
        ...
        section(
            ...
            "si la passerelle répond 504 (délai dépassé), Pod continue de son côté : "
            f"l'appli attend jusqu'à {cfg.CHUNK_VERIFY_TIMEOUT_S // 60} min que la vidéo "
            "apparaisse, puis poursuit. Sur un 502 ou un 503 (erreur du serveur), la "
            f"vidéo n'est en général PAS créée : l'attente est limitée à "
            f"{cfg.CHUNK_VERIFY_TIMEOUT_502_S // 60} min. Dans les deux cas, 🛑 Interrompre "
            "arrête l'attente, et le Journal indique le repère à rechercher sur Pod.",
